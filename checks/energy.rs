...
use librint::cint1e::{cint1e_nuc_cart, cint1e_ovlp_cart};
use librint::cint2e::cint2e_cart;
use librint::cint_bas::CINTcgto_cart;
use librint::intor1::cint1e_kin_cart;
use librint::scf::{density, norm};
...
#[no_mangle]
pub fn nelec(atm: &Vec<i32>) -> usize {
    // dbg!(atm);
    let mut nelec = 0;
...
pub fn E(atm: &mut Vec<i32>, bas: &mut Vec<i32>, env: &mut Vec<f64>, P: &mut Vec<f64>) -> f64 {
...
fn set_molecule(path: &str) -> (usize, usize, Vec<i32>, Vec<i32>, Vec<f64>) {
...
pub fn test_path(path: &str, exp: &str) {
    ...
    if (E - E_exp).abs() > epsilon {
        panic!(
            "Test failed for molecule '{}' exp={} actual={}",
            path, E_exp, E
        );
    }
}
