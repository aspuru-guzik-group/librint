...
unsafe extern "C" fn R_dnode(
    ...
    let mut m: i32 = 0;
    let mut n: i32 = 0;
    x1init = 0 as libc::c_double;
    p1init = *a.offset(0 as isize);
    m = 0 as i32;
    while m < order {
        x0 = x1init;
        p0 = p1init;
        ...
        if !(p1init == 0 as libc::c_double) {
            if p0 * p1init > 0 as libc::c_double {
                fprintf(
                    stderr,
            ...
            if p1 == 0 as libc::c_double {
                *roots.offset(m as isize) = x1;
            } else if p0 == 0 as libc::c_double {
                *roots.offset(m as isize) = x0;
                ...
                while fabs(x1 - x0) > x1 * accrt {
                    ...
                    if pi == 0 as libc::c_double {
                        break;
                    }
                    if p0 * pi <= 0 as libc::c_double {
                        x1 = xi;
                        p1 = pi;
...
unsafe extern "C" fn _qr_step(
    ...
    if v == 0 as libc::c_double {
        v = 1 as libc::c_double;
        c = 1 as libc::c_double;
    ...
    while j < n1 - 2 as i32 {
        ...
        *A.offset((j2 * nroots + j) as isize) = 0 as libc::c_double;
        if v == 0 as libc::c_double {
            v = 1 as libc::c_double;
            c = 1 as libc::c_double;
            s = 0 as libc::c_double;
...
unsafe extern "C" fn _hessenberg_qr(
    ...
    while ic < nroots * maxits {
        ...
        k1 = k + 1 as i32;
        if k1 < n1 {
            *A.offset((k1 * nroots + k) as isize) = 0 as libc::c_double;
            n0 = k1;
            its = 0 as i32;
            if n0 + 1 as i32 >= n1 {
                n0 = 0 as i32;
...
pub unsafe extern "C" fn _CINT_polynomial_roots(
    ...
    if nroots == 1 as i32 {
        *roots
            .offset(
                0 as isize,
            ) = -*cs.offset(2 as isize)
            / *cs.offset(3 as isize);
        return 0 as i32;
    ...
    while i < nroots * nroots {
        A[i as usize] = 0 as libc::c_double;
        i += 1;
        i;
    }
    i = 0 as i32;
    ...
    if err == 0 as i32 {
        ...
        *roots
        ...
        i = 2 as i32;
        while i < nroots {
            *roots.offset(i as isize) = 1 as libc::c_double;
            i += 1;
            i;
        }
        k = 2 as i32;
