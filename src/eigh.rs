...
unsafe fn _dlarrk(
    ...
    while it < 1000_i32 {
                    ...
                    })
        {
            ...
            i = 1_i32;
            while i < n {
                tmp1 = *diag.offset(i as isize) - *e2.offset((i - 1_i32) as isize) / tmp1 - mid;
                if tmp1 <= 0.0f64 {
                    negcnt += 1;
                    negcnt;
                }
...
unsafe fn _dlarrc(
    ...
    lpivot = *diag.offset(0_isize) - vl;
    rpivot = *diag.offset(0_isize) - vu;
    if lpivot <= 0.0f64 {
        left_count += 1;
        left_count;
    }
    ...
    while i < n - 1_i32 {
        ...
        lpivot = *diag.offset((i + 1_i32) as isize) - vl - tmp / lpivot;
        rpivot = *diag.offset((i + 1_i32) as isize) - vu - tmp / rpivot;
        if lpivot <= 0.0f64 {
            left_count += 1;
            left_count;
        }
...
unsafe fn _dlasq4(
    ...
    let mut i: i32 = 0;
    if n0init == n0 {
        if *dmin.offset(0_isize) == *dn.offset(0_isize) {
            if *dmin.offset(1_isize) == *dn.offset(1_isize) {
                b1 = (*qvecp.offset((n0 - 1_i32) as isize) * *evecp.offset((n0 - 2_i32) as isize))
                    .sqrt();
                b2 = (*qvecp.offset((n0 - 2_i32) as isize) * *evecp.offset((n0 - 3_i32) as isize))
                    .sqrt();
                a2 = *qvecp.offset((n0 - 2_i32) as isize) + *evecp.offset((n0 - 2_i32) as isize);
                gap2 = *dmin.offset(2_isize) - a2 - *dmin.offset(2_isize) * 0.25f64;
                if gap2 > b2 {
                    gap1 = a2 - *dn.offset(0_isize) - b2 / gap2 * b2;
                } else {
                    gap1 = a2 - *dn.offset(0_isize) - (b1 + b2);
                }
                if gap1 > b1 {
                    s = if *dn.offset(0_isize) - b1 / gap1 * b1 > *dmin.offset(0_isize) * 0.5f64 {
                        *dn.offset(0_isize) - b1 / gap1 * b1
                    } else {
                        *dmin.offset(0_isize) * 0.5f64
                    };
                } else {
                    s = 0.0f64;
                    if *dn.offset(0_isize) > b1 {
                        s = *dn.offset(0_isize) - b1;
                    }
                    if a2 > b1 + b2 {
                        s = if s < a2 - (b1 + b2) {
                            s
                    ...
                    s = if s > *dmin.offset(0_isize) * 0.333f64 {
                        s
                    } else {
                        *dmin.offset(0_isize) * 0.333f64
                    };
                }
            } else {
                if *evecp.offset((n0 - 2_i32) as isize) > *qvecp.offset((n0 - 2_i32) as isize) {
                    return 0_i32;
                }
                b2 = *evecp.offset((n0 - 2_i32) as isize) / *qvecp.offset((n0 - 2_i32) as isize);
                a2 = b2;
                i = n0 - 3_i32;
                while i >= i0 {
                    b1 = b2;
                ...
                a2 *= 1.05f64;
                if a2 < 0.563f64 {
                    s = *dn.offset(0_isize) * (1.0f64 - (a2).sqrt()) / (a2 + 1.0f64);
                } else {
                    s = *dmin.offset(0_isize) * 0.25f64;
                }
            }
        } else if *dmin.offset(0_isize) == *dn.offset(1_isize) {
            if *evec1p.offset((n0 - 2_i32) as isize) > *qvec1p.offset((n0 - 1_i32) as isize)
                || *evecp.offset((n0 - 3_i32) as isize) > *qvecp.offset((n0 - 3_i32) as isize)
            {
                return 0_i32;
            }
            a2 = *evec1p.offset((n0 - 2_i32) as isize) / *qvec1p.offset((n0 - 1_i32) as isize);
            b2 = *evecp.offset((n0 - 3_i32) as isize) / *qvecp.offset((n0 - 3_i32) as isize);
            a2 += b2;
            i = n0 - 4_i32;
            while i >= i0 {
                if b2 == 0.0f64 {
            ...
            a2 *= 1.05f64;
            if a2 < 0.563f64 {
                s = *dn.offset(1_isize) * (1.0f64 - (a2).sqrt()) / (a2 + 1.0f64);
            } else {
                s = *dmin.offset(0_isize) * 0.25f64;
            }
        } else if *dmin.offset(0_isize) == *dn.offset(2_isize) {
            if *evec1p.offset((n0 - 3_i32) as isize) > *qvec1p.offset((n0 - 2_i32) as isize)
                || *evec1p.offset((n0 - 2_i32) as isize) > *qvec1p.offset((n0 - 1_i32) as isize)
            {
                return 0_i32;
            }
            a2 = *evec1p.offset((n0 - 3_i32) as isize) / *qvec1p.offset((n0 - 2_i32) as isize)
                * (*evec1p.offset((n0 - 2_i32) as isize) / *qvec1p.offset((n0 - 1_i32) as isize)
                    + 1.0f64);
            if n0 - i0 > 3_i32 {
                b2 = *evecp.offset((n0 - 4_i32) as isize) / *qvecp.offset((n0 - 4_i32) as isize);
                a2 += b2;
                i = n0 - 5_i32;
                while i >= i0 {
                    b1 = b2;
            ...
            if a2 < 0.563f64 {
                s = *dn.offset(2_isize) * (1.0f64 - (a2).sqrt()) / (a2 + 1.0f64);
            } else {
                s = *dmin.offset(0_isize) * 0.25f64;
            }
        } else {
    ...
    } else if n0init == n0 + 1_i32 {
        if *dmin.offset(1_isize) == *dn.offset(1_isize)
            && *dmin.offset(2_isize) == *dn.offset(2_isize)
        {
            if *evecp.offset((n0 - 2_i32) as isize) > *qvecp.offset((n0 - 2_i32) as isize) {
                return 0_i32;
            }
            s = *dmin.offset(1_isize) * 0.333f64;
            b1 = *evecp.offset((n0 - 2_i32) as isize) / *qvecp.offset((n0 - 2_i32) as isize);
            b2 = b1;
            if b2 != 0.0f64 {
                i = n0 - 3_i32;
                while i >= i0 {
            ...
            a2 = *dmin.offset(1_isize) / (b2 * 1.05f64 + 1.0f64);
            b2 = (b2 * 1.05f64).sqrt();
            gap2 = *dmin.offset(2_isize) * 0.5f64 - a2;
        ...
        } else if *dmin.offset(1_isize) == *dn.offset(1_isize) {
            s = *dmin.offset(1_isize) * 0.5f64;
        } else {
            s = *dmin.offset(1_isize) * 0.25f64;
        }
    } else if n0init == n0 + 2_i32 {
        if *dmin.offset(2_isize) == *dn.offset(2_isize)
            && *evecp.offset((n0 - 2_i32) as isize) * 2.0f64 < *qvecp.offset((n0 - 2_i32) as isize)
        {
            if *evecp.offset((n0 - 2_i32) as isize) > *qvecp.offset((n0 - 2_i32) as isize) {
                return 0_i32;
            }
            ...
            if b2 != 0.0f64 {
                i = n0 - 2_i32;
                while i > i0 {
                    if *evecp.offset((i - 1_i32) as isize) > *qvecp.offset((i - 1_i32) as isize) {
                        return 0_i32;
                    }
                    b1 *= *evecp.offset((i - 1_i32) as isize) / *qvecp.offset((i - 1_i32) as isize);
                    b2 += b1;
                    if b1 * 100.0f64 < b2 {
                        break;
                    }
            ...
            s = *dmin.offset(2_isize) * 0.333f64;
            a2 = *dmin.offset(2_isize) / (b2 * 1.05f64 + 1.0f64);
            b2 = (b2 * 1.05f64).sqrt();
            gap2 = *qvecp.offset((n0 - 2_i32) as isize) + *evecp.offset((n0 - 3_i32) as isize)
                - (*qvecp.offset((n0 - 3_i32) as isize) * *evecp.offset((n0 - 3_i32) as isize))
                    .sqrt()
                - a2;
            if gap2 > 0.0f64 && gap2 > b2 * a2 {
                s = if s > a2 * (1.0f64 - a2 * 1.01f64 * (b2 / gap2) * b2) {
...
unsafe fn _dlasq5(
    ...
    while j < n0 - 3_i32 {
        *qvec1p.offset(j as isize) = diag + *evecp.offset(j as isize);
        temp = *qvecp.offset((j + 1_i32) as isize) / *qvec1p.offset(j as isize);
        diag = diag * temp - tau;
        if diag < tol {
            diag = 0.0f64;
        }
    ...
    *dn.offset(2_isize) = diag;
    j = n0 - 3_i32;
    *qvec1p.offset(j as isize) = diag + *evecp.offset(j as isize);
    temp = *qvecp.offset((j + 1_i32) as isize) / *qvec1p.offset(j as isize);
    *evec1p.offset(j as isize) = *evecp.offset(j as isize) * temp;
    diag = diag * temp - tau;
    *dn.offset(1_isize) = diag;
    j = n0 - 2_i32;
    *qvec1p.offset(j as isize) = diag + *evecp.offset(j as isize);
    temp = *qvecp.offset((j + 1_i32) as isize) / *qvec1p.offset(j as isize);
    *evec1p.offset(j as isize) = *evecp.offset(j as isize) * temp;
    diag = diag * temp - tau;
    *dn.offset(0_isize) = diag;
    *qvec1p.offset((n0 - 1_i32) as isize) = diag;
    *dmin.offset(2_isize) = diag_min;
    *dmin.offset(1_isize) = if *dmin.offset(2_isize) < *dn.offset(1_isize) {
        *dmin.offset(2_isize)
    } else {
        *dn.offset(1_isize)
    };
    *dmin.offset(0_isize) = if *dmin.offset(1_isize) < *dn.offset(0_isize) {
        *dmin.offset(1_isize)
    } else {
        *dn.offset(0_isize)
    };
...
unsafe fn _dlasq2(n: i32, work: *mut f64, diag: *mut f64, diag_off: *mut f64) -> i32 {
    ...
    *qvec.offset(0_isize) = fabs(*diag.offset((n - 1_i32) as isize));
    *qvec1.offset((n - 1_i32) as isize) = 0.0f64;
    *evec.offset((n - 1_i32) as isize) = 0.0f64;
    *evec1.offset((n - 1_i32) as isize) = 0.0f64;
    if *qvec.offset(0_isize) < *qvec.offset((n - 1_i32) as isize) * 1.5f64 {
        i = 0_i32;
        j = n - 1_i32;
        while i < n / 2_i32 {
            ...
            *evec.offset(i as isize) = *evec.offset((j - 1_i32) as isize);
            *evec.offset((j - 1_i32) as isize) = temp;
            i += 1;
            i;
            j -= 1;
            j;
    ...
    diag_sum = *qvec.offset(0_isize);
    i = 0_i32;
    while i < n - 1_i32 {
        temp = diag_sum + *evec.offset(i as isize);
        *qvec1.offset(i as isize) = temp;
        *evec1.offset(i as isize) =
            *qvec.offset((i + 1_i32) as isize) * (*evec.offset(i as isize) / temp);
        diag_sum = *qvec.offset((i + 1_i32) as isize) * (diag_sum / temp);
        i += 1;
        i;
    }
    *qvec1.offset((n - 1_i32) as isize) = diag_sum;
    diag_sum = *qvec1.offset(0_isize);
    i = 0_i32;
    while i < n - 1_i32 {
        temp = diag_sum + *evec1.offset(i as isize);
        *qvec.offset(i as isize) = temp;
        *evec.offset(i as isize) =
            *qvec1.offset((i + 1_i32) as isize) * (*evec1.offset(i as isize) / temp);
        diag_sum = *qvec1.offset((i + 1_i32) as isize) * (diag_sum / temp);
        i += 1;
        i;
    }
    *qvec.offset((n - 1_i32) as isize) = diag_sum;
    n0 = n;
    tau = 0.0f64;
    itry = 0_i32;
    while n0 > 0_i32 {
        ...
        sigma = -*evec.offset((n0 - 1_i32) as isize);
        if sigma < 0.0f64 {
            return 1_i32;
        }
        emax = 0.0f64;
        qmin = *qvec.offset((n0 - 1_i32) as isize);
        i = n0 - 1_i32;
        while i > 0_i32 {
            if *evec.offset((i - 1_i32) as isize) <= 0.0f64 {
                break;
            }
            if qmin >= emax * 4.0f64 {
                ...
                emax = if emax > *evec.offset((i - 1_i32) as isize) {
                    emax
                } else {
                    *evec.offset((i - 1_i32) as isize)
                };
            }
            i -= 1;
            i;
        ...
        evecp = evec;
        evec1p = evec1;
        if qmin < 0 as f64 || emax < 0 as f64 {
            eprintln!("dlasq2: qmin < 0 or emax < 0");
            return 1_i32;
        }
        dmin[0_i32 as usize] = -if 0.0f64 > qmin - 2.0f64 * (qmin * emax).sqrt() {
            0.0f64
        } else {
            ...
            while n0 > i0 {
                    ...
                    || *evec1p.offset(n2 as isize) < tol2 * *qvecp.offset(n2 as isize)
                {
                    *qvec.offset(n1 as isize) = *qvecp.offset(n1 as isize) + sigma;
                    ...
                    if !(n2 == i0
                        || *evecp.offset((n1 - 2_i32) as isize) < tol2 * sigma
                        || *evec1p.offset((n1 - 2_i32) as isize)
                            < tol2 * *qvecp.offset((n1 - 2_i32) as isize))
                    {
                        break;
                    }
...
unsafe fn _compute_eigenvalues(
    ...
    if n == 1_i32 {
        *w.offset(0_isize) = *diag.offset(0_isize);
        *werr.offset(0_isize) = 0.0f64;
    ...
    eold = 0.0f64;
    *diag_off1.offset((n - 1_i32) as isize) = 0.0f64;
    i = 0_i32;
    while i < n {
    ...
    *diag_off1.offset((n - 1_i32) as isize) = 0.0f64;
    ...
    iinfo = _dlarrk(n, 1_i32, gl, gu, diag, e2, rtl, &mut tmp, &mut tmp1);
    if iinfo != 0_i32 {
        return -1_i32;
    }
    ...
    idum = 0_i32;
    while idum < 6_i32 {
        dpivot = *diag.offset(0_isize) - sigma;
        *work.offset(0_isize) = dpivot;
        dmax = fabs(*work.offset(0_isize));
        i = 0_i32;
        while i < n - 1_i32 {
            *work.offset((n * 2_i32 + i) as isize) = 1.0f64 / *work.offset(i as isize);
            tmp = *diag_off1.offset(i as isize) * *work.offset((n * 2_i32 + i) as isize);
            *work.offset((n + i) as isize) = tmp;
            dpivot =
                *diag.offset((i + 1_i32) as isize) - sigma - tmp * *diag_off1.offset(i as isize);
            *work.offset((i + 1_i32) as isize) = dpivot;
            dmax = if dmax > fabs(dpivot) {
                dmax
            } else {
                fabs(dpivot)
            };
    ...
    *diag_off1.offset((n - 1_i32) as isize) = sigma;
    ip = 0_i32;
    while ip < n {
    ...
    if sgndef > 0.0f64 {
        i = 0_i32;
        while i < n {
            *w.offset(i as isize) = *work.offset((n - 1_i32 - i) as isize);
            i += 1;
            i;
        }
    } else {
    ...
    while i < n - 1_i32 {
        *wgap.offset(i as isize) = if 0.0f64
            > *w.offset((i + 1_i32) as isize)
                - *werr.offset((i + 1_i32) as isize)
    ...
    *wgap.offset(-1_i32 as isize) = if 0.0f64 > *w.offset(0_isize) - *werr.offset(0_isize) - gl {
        0.0f64
    } else {
        *w.offset(0_isize) - *werr.offset(0_isize) - gl
    };
    *wgap.offset((n - 1_i32) as isize) = if 0.0f64
        > gu - sigma - (*w.offset((n - 1_i32) as isize) + *werr.offset((n - 1_i32) as isize))
    {
        0.0f64
    } else {
        gu - sigma - (*w.offset((n - 1_i32) as isize) + *werr.offset((n - 1_i32) as isize))
...
unsafe fn _dlarrf(
    ...
    while ktry < 2_i32 {
        s = -lsigma;
        *dplus.offset(0_isize) = *diag.offset(0_isize) + s;
        max1 = fabs(*dplus.offset(0_isize));
        i = 0_i32;
        while i < n - 1_i32 {
            tmp = *ld.offset(i as isize) / *dplus.offset(i as isize);
            *lplus.offset(i as isize) = tmp;
            s = s * tmp * *diag_off1.offset(i as isize) - lsigma;
            *dplus.offset((i + 1_i32) as isize) = *diag.offset((i + 1_i32) as isize) + s;
            max1 = if max1 > fabs(*dplus.offset((i + 1_i32) as isize)) {
                max1
            } else {
                fabs(*dplus.offset((i + 1_i32) as isize))
            };
            i += 1;
            i;
        }
...
unsafe fn _dlaneg(n: i32, diag: *mut f64, lld: *mut f64, sigma: f64, twist_index: i32) -> i32 {
    ...
    p = *diag.offset((n - 1_i32) as isize) - sigma;
    j = n - 2_i32;
    while j >= twist_index - 1_i32 {
    ...
    let cvrgd: f64 = if rtol1 * *wgap.offset(ifirst as isize)
        > rtol2
            * (if fabs(*w.offset(ifirst as isize)) > fabs(*w.offset((ilast - 1_i32) as isize)) {
                ...
                fabs(*w.offset((ilast - 1_i32) as isize))
            }) {
...
unsafe fn _dlar1v(
    ...
    if *twist_index == -1_i32 {
        ...
        *twist_index = 0_i32;
        ...
        r1 = *twist_index;
        r2 = *twist_index + 1_i32;
    }
    neg2 = 0_i32;
    s = *diag.offset((n - 1_i32) as isize) - lambda;
    *work_p.offset((n - 1_i32) as isize) = s;
    i = n - 2_i32;
    while i >= r1 {
    ...
    *vec.offset(*twist_index as isize) = 1.0f64;
    ...
    i = *twist_index - 1_i32;
    while i >= 0_i32 {
        tmp = -(*lplus.offset(i as isize) * *vec.offset((i + 1_i32) as isize));
        ztz += tmp * tmp;
    ...
    while i < n - 1_i32 {
        ...
        ztz += tmp * tmp;
        *vec.offset((i + 1_i32) as isize) = tmp;
        i += 1;
        i;
    }
    tmp = 1.0f64 / ztz;
...
unsafe fn _compute_eigenvectors(
    ...
    buf_lld = work.offset((n * 2_i32) as isize);
    buf_wrk = work.offset((n * 3_i32) as isize);
    i = 0_i32;
    while i < n * 6_i32 {
    ...
    i = 0_i32;
    while i < n {
        *twist_indices.offset(i as isize) = 0_i32;
        i += 1;
        i;
    }
    ...
    ncluster = 1_i32;
    *old_cluster_range.offset(0_isize) = 0_i32;
    *old_cluster_range.offset(1_isize) = n;
    idone = 0_i32;
    ndepth = 0_i32;
    while ndepth < n {
        if idone == n {
        ...
        while icls < ncluster1 {
            oldfst = *old_cluster_range.offset((icls * 2_i32) as isize);
            oldlst = *old_cluster_range.offset((icls * 2_i32 + 1_i32) as isize);
            if ndepth > 0_i32 {
                i = 0_i32;
                while i < n {
                ...
                while i < n - 1_i32 {
                    *diag_off1.offset(i as isize) =
                        *vec.offset(((oldfst + 1_i32) * n + i) as isize);
                    i += 1;
                    i;
                }
                sigma = *vec.offset(((oldfst + 2_i32) * n - 1_i32) as isize);
            }
            j = 0_i32;
            while j < n - 1_i32 {
            ...
            while newlst <= oldlst {
                if !(newlst < oldlst
                    && *wgap.offset((newlst - 1_i32) as isize)
                        < 0.001f64 * fabs(*buf_w.offset((newlst - 1_i32) as isize)))
                {
                    if newlst - newfst > 1_i32 {
                        _dlarrb(
                            n,
                            diag,
                            buf_lld,
                        ...
                        iinfo = _dlarrf(
                            ...
                            wgap,
                            werr,
                            *wgap.offset((newfst - 1_i32) as isize),
                            &mut tau,
                            vec.offset((newfst * n) as isize),
                            vec.offset(((newfst + 1_i32) * n) as isize),
                        );
                        if iinfo != 0_i32 {
                            return -2_i32;
                        }
                        *vec.offset(((newfst + 2_i32) * n - 1_i32) as isize) = sigma + tau;
                        k = newfst;
                        while k < newlst {
                        ...
                        *new_cluster_range.offset((ncluster * 2_i32) as isize) = newfst;
                        *new_cluster_range.offset((ncluster * 2_i32 + 1_i32) as isize) = newlst;
                        ncluster += 1;
                        ncluster;
                    } else {
                        ...
                        if newfst == 0_i32 || newfst + 1_i32 == n {
                            gaptol = 0 as f64;
                        } else {
                            gaptol = gap * eps;
                        }
                        ...
                        bstres = 1e307f64;
                        bstw = 0 as f64;
                        *twist_indices.offset(newfst as isize) = -1_i32;
                        iter = 0_i32;
                        while iter < 6_i32 {
                            _dlar1v(
                                n,
                        ...
                        *w.offset(newfst as isize) = lambda + sigma;
                        if newfst > 0_i32 {
                            *wgap.offset((newfst - 1_i32) as isize) = if *wgap
                                .offset((newfst - 1_i32) as isize)
                                > *w.offset(newfst as isize)
                                    - *werr.offset(newfst as isize)
                                    - *w.offset((newfst - 1_i32) as isize)
                                    - *werr.offset((newfst - 1_i32) as isize)
                            {
                                *wgap.offset((newfst - 1_i32) as isize)
                            } else {
                                *w.offset(newfst as isize)
...
unsafe fn _dlaev2(eig: *mut f64, vec: *mut f64, diag: *mut f64, diag_off1: *mut f64) -> i32 {
    ...
    *eig.offset(0_isize) = rt2;
    *eig.offset(1_isize) = rt1;
    *vec.offset(0_isize) = -sn1;
    *vec.offset(1_isize) = cs1;
    *vec.offset(2_isize) = cs1;
...
pub unsafe fn _CINTdiagonalize(
    ...
    if n == 0_i32 {
        ...
        *eig.offset(0_isize) = *diag.offset(0_isize);
        *vec.offset(0_isize) = 1.0f64;
        return 0_i32;
    } else if n == 2_i32 {
