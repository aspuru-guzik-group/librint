...
pub unsafe fn CINTinit_int1e_EnvVars(
    ...
    (*envs).x_ctr[0 as i32 as usize] = *bas.offset((8 * i_sh + 3 as i32) as isize);
    (*envs).x_ctr[1 as i32 as usize] = *bas.offset((8 * j_sh + 3 as i32) as isize);
    ...
    (*envs).common_factor = 1 as f64;
    if *env.offset(0) == 0 as f64 {
        (*envs).expcutoff = 60 as f64;
    } else {
        (*envs).expcutoff = MAX(40 as f64, *env.offset(0)) as f64;
    }
    (*envs).li_ceil = (*envs).i_l + ng[0];
    (*envs).lj_ceil = (*envs).j_l + ng[1];
    (*envs).ri = env.offset(
        *atm.offset((6 as i32 * *bas.offset((8 * i_sh + 0 as i32) as isize) + 1 as i32) as isize)
            as isize,
    );
    (*envs).rj = env.offset(
        *atm.offset((6 as i32 * *bas.offset((8 * j_sh + 0 as i32) as isize) + 1 as i32) as isize)
            as isize,
    );
    (*envs).gbits = ng[4];
    (*envs).ncomp_e1 = ng[5];
    ...
    if ibase != 0 {
        ...
        (*envs).rirj[0 as i32 as usize] = *((*envs).ri).offset(0) - *((*envs).rj).offset(0);
        (*envs).rirj[1 as i32 as usize] = *((*envs).ri).offset(1) - *((*envs).rj).offset(1);
        (*envs).rirj[2 as i32 as usize] = *((*envs).ri).offset(2) - *((*envs).rj).offset(2);
    } else {
        dli = (*envs).li_ceil + 1 as i32;
        dlj = (*envs).li_ceil + (*envs).lj_ceil + 1 as i32;
        (*envs).rirj[0 as i32 as usize] = *((*envs).rj).offset(0) - *((*envs).ri).offset(0);
        (*envs).rirj[1 as i32 as usize] = *((*envs).rj).offset(1) - *((*envs).ri).offset(1);
        (*envs).rirj[2 as i32 as usize] = *((*envs).rj).offset(2) - *((*envs).ri).offset(2);
    }
...
#[no_mangle]
pub unsafe fn CINTg1e_index_xyz(idx: *mut i32, envs: &CINTEnvVars) {
...
#[no_mangle]
pub unsafe fn CINTg1e_ovlp(mut g: *mut f64, envs: &CINTEnvVars) -> i32 {
    ...
    *gx.offset(0) = 1 as f64;
    *gy.offset(0) = 1 as f64;
    *gz.offset(0) = (*envs).fac[0 as usize]
    ...
    rijrx[0 as i32 as usize] = *rij.offset(0) - *rx.offset(0);
    rijrx[1 as i32 as usize] = *rij.offset(1) - *rx.offset(1);
    rijrx[2 as i32 as usize] = *rij.offset(2) - *rx.offset(2);
    *gx.offset(di as isize) = rijrx[0 as usize] * *gx.offset(0);
    *gy.offset(di as isize) = rijrx[1 as usize] * *gy.offset(0);
    *gz.offset(di as isize) = rijrx[2 as usize] * *gz.offset(0);
    ...
    while j <= lj {
        ...
        while i <= nmax - j {
            *gx.offset(n as isize) = *gx.offset((n + di - dj) as isize)
                + *rirj.offset(0) * *gx.offset((n - dj) as isize);
            *gy.offset(n as isize) = *gy.offset((n + di - dj) as isize)
                + *rirj.offset(1) * *gy.offset((n - dj) as isize);
            *gz.offset(n as isize) = *gz.offset((n + di - dj) as isize)
                + *rirj.offset(2) * *gz.offset((n - dj) as isize);
            i += 1;
            i;
            n += di;
        }
...
#[no_mangle]
pub unsafe fn CINTg1e_nuc(mut g: *mut f64, envs: &CINTEnvVars, mut nuc_id: i32) -> i32 {
    ...
    if nuc_id < 0 as i32 {
        fac1 = 2 as f64 * 3.14159265358979323846f64 * (*envs).fac[0 as usize] * tau / aij;
        ...
        fac1 = 2 as f64
            * 3.14159265358979323846f64
            * -(*atm.offset((0 as i32 + nuc_id * 6 as i32) as isize)).abs() as f64
            * (*envs).fac[0 as usize]
            * tau
            / aij;
        cr = env.offset(*atm.offset((6 as i32 * nuc_id + 1 as i32) as isize) as isize);
    }
    crij[0] = *cr.offset(0) - *rij.offset(0);
    crij[1] = *cr.offset(1) - *rij.offset(1);
    crij[2] = *cr.offset(2) - *rij.offset(2);
    x = aij * tau * tau * SQUARE(crij.as_ptr()) as f64;
    ...
    let mut rijrx: f64 = *rij.offset(0) - *rx.offset(0);
    let mut rijry: f64 = *rij.offset(1) - *rx.offset(1);
    let mut rijrz: f64 = *rij.offset(2) - *rx.offset(2);
    let mut aij2: f64 = 0.5f64 / aij;
    let mut ru: f64 = 0.;
    let mut rt: f64 = 0.;
    let mut r0: f64 = 0.;
    ...
    n = 0 as i32;
    while n < nrys_roots {
        ru = tau * tau * u[n as usize] / (1 as f64 + u[n as usize]);
        rt = aij2 - aij2 * ru;
        r0 = rijrx + ru * crij[0];
        r1 = rijry + ru * crij[1];
        r2 = rijrz + ru * crij[2];
...
pub unsafe fn CINTnabla1i_1e(
    ...
    while k <= lk {
        ...
        while j <= lj {
            ptr = dj * j + dk * k;
            *fx.offset(ptr as isize) = ai2 * *gx.offset((ptr + 1 as i32) as isize);
            *fy.offset(ptr as isize) = ai2 * *gy.offset((ptr + 1 as i32) as isize);
            *fz.offset(ptr as isize) = ai2 * *gz.offset((ptr + 1 as i32) as isize);
            i = 1 as i32;
            while i <= li {
...
pub unsafe fn CINTx1i_1e(
    ...
    while k <= lk {
        ...
        while j <= lj {
            ...
            while i <= ptr + li {
                *fx.offset(i as isize) =
                    *gx.offset((i + 1 as i32) as isize) + *ri.offset(0) * *gx.offset(i as isize);
                *fy.offset(i as isize) =
                    *gy.offset((i + 1 as i32) as isize) + *ri.offset(1) * *gy.offset(i as isize);
                *fz.offset(i as isize) =
                    *gz.offset((i + 1 as i32) as isize) + *ri.offset(2) * *gz.offset(i as isize);
                i += 1;
                i;
            }
            j += 1;
...
pub unsafe fn CINTx1j_1e(
    ...
    while k <= lk {
        ...
        while j <= lj {
            ...
            while i <= ptr + li {
                *fx.offset(i as isize) =
                    *gx.offset((i + dj) as isize) + *rj.offset(0) * *gx.offset(i as isize);
                *fy.offset(i as isize) =
                    *gy.offset((i + dj) as isize) + *rj.offset(1) * *gy.offset(i as isize);
                *fz.offset(i as isize) =
                    *gz.offset((i + dj) as isize) + *rj.offset(2) * *gz.offset(i as isize);
                i += 1;
                i;
            }
            j += 1;
...
pub unsafe fn CINTx1k_1e(
    ...
    while k <= lk {
        ...
        while j <= lj {
            ...
            while i <= ptr + li {
                *fx.offset(i as isize) =
                    *gx.offset((i + dk) as isize) + *rk.offset(0) * *gx.offset(i as isize);
                *fy.offset(i as isize) =
                    *gy.offset((i + dk) as isize) + *rk.offset(1) * *gy.offset(i as isize);
                *fz.offset(i as isize) =
                    *gz.offset((i + dk) as isize) + *rk.offset(2) * *gz.offset(i as isize);
                i += 1;
                i;
            }
            j += 1;
