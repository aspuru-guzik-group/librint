...
unsafe fn fmt1_gamma_inc_like(f: *mut f64, t: f64, m: i32) {
    ...
    let mut bi: f64 = 0.;
    let e: f64 = 0.5f64 * exp(-t);
    let mut x: f64 = e;
    let mut s: f64 = e;
    let tol: f64 = 2.220_446_049_250_313e-16_f64 * 0.5f64 * e;
    bi = b + 1.0f64;
    while x > tol {
        x *= t / bi;
        s += x;
    ...
    *f.offset(m as isize) = s / b;
    ...
    while i > 0_i32 {
        b -= 1.0f64;
        *f.offset((i - 1_i32) as isize) = (e + t * *f.offset(i as isize)) / b;
        i -= 1;
        i;
    }
}
pub unsafe fn gamma_inc_like(f: *mut f64, t: f64, m: i32) {
    if t < TURNOVER_POINT[m as usize] {
        ...
        let mut i: i32 = 0;
        let tt: f64 = sqrt(t);
        *f.offset(0_isize) = 0.886_226_925_452_758_f64 / tt * erf(tt);
        if m > 0_i32 {
            let e: f64 = exp(-t);
            let b: f64 = 0.5f64 / t;
            i = 1_i32;
            while i <= m {
                *f.offset(i as isize) =
                    b * ((2_i32 * i - 1_i32) as f64 * *f.offset((i - 1_i32) as isize) - e);
                i += 1;
                i;
            }
        }
...
pub unsafe fn lgamma_inc_like(f: *mut f64, t: f64, m: i32) {
    if t < TURNOVER_POINT[m as usize] {
        ...
        let mut i: i32 = 0;
        let tt: f64 = sqrtl(t);
        *f.offset(0_isize) = 0.886_226_925_452_758 / tt * erfl(tt);
        if m > 0_i32 {
            let e: f64 = expl(-t);
            let b: f64 = 0.5f64 / t;
            i = 1_i32;
...
pub unsafe fn fmt1_erfc_like(f: *mut f64, t: f64, lower: f64, m: i32) {
    ...
    let mut delta: f64 = s;
    let tol: f64 = 2.220_446_049_250_313e-16_f64 * 0.5f64 * fabs(delta);
    bi = b + 1.0f64;
...
pub unsafe fn fmt_erfc_like(f: *mut f64, t: f64, lower: f64, m: i32) {
    ...
    if t < TURNOVER_POINT[m as usize] {
        ...
        let tt: f64 = sqrt(t);
        let mut val: f64 = 0.886_226_925_452_758_f64 / tt * (erfc(lower * tt) - erfc(tt));
        *f.offset(0_isize) = val;
        if m > 0_i32 {
            ...
            i = 0_i32;
            while i < m {
                val = b * ((2_i32 * i + 1_i32) as f64 * val - e + e1);
                e1 *= lower2;
                *f.offset((i + 1_i32) as isize) = val;
                i += 1;
                i;
...
pub unsafe fn fmt_lerfc_like(f: *mut f64, t: f64, lower: f64, m: i32) {
    ...
    if t < TURNOVER_POINT[m as usize] {
        ...
        let tt: f64 = sqrtl(t);
        let mut val: f64 = 0.886_226_925_452_758 / tt * (erfcl(lower * tt) - erfcl(tt));
        *f.offset(0_isize) = val;
        if m > 0_i32 {
            let e: f64 = expl(-t);
...
pub unsafe fn fmt1_lerfc_like(f: *mut f64, t: f64, lower: f64, m: i32) {
    ...
    let mut div: f64 = 1.0f64;
    let mut delta: f64 = s;
    let tol: f64 = 2.0e-20f64 * fabsl(delta);
    bi = b + 1.0f64;
