...
fn main() {
    ...
    time_reverse_element_mode(&mut atm, natm, &mut bas, nbas, &mut env, env2.len(), &mut dS_rev);
    time_reverse_matrix_mode(&mut atm, &mut bas, &mut env, env2.len(), &mut dS_matrix_rev);

    time_forward_element_mode(&mut atm, natm, &mut bas, nbas, &mut env, env2.len(), &mut dS_for);
    ...
    for i in 0..dS_rev.len() {
        ...
        if (dS_rev[i] - dS_for[i]).abs() > 1e-10 {
            println!("Mismatch at index {}: dS_rev = {}, dS_for = {}", i, dS_rev[i], dS_for[i]);
            mismatches += 1;
        }
    }
    if mismatches == 0 {
