...
fn main() {
    ...
    for i in 0..nbas {
        for j in 0..nbas {
            ...
            ovlp_cart(&mut buf, &mut shls, &mut atm, &mut bas, &mut env);

            dovlp_cart(
                &mut buf, &mut dbuf, &mut shls, &mut atm, &mut bas, &mut env, &mut denv,
            );
    ...
    for i in 0..nbas {
        for j in 0..nbas {
            ...
            ovlp_sph(&mut buf, &mut shls, &mut atm, &mut bas, &mut env);
